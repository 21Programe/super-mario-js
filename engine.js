import { state, PLAYER_STATES } from './state.js';
import { WORLDS, parseLevel } from './levels.js';
import { Input } from './input.js';
import { updatePlayer, bulletMove, aabb, PHYS, moveBody } from './physics.js';
...
export class Camera {
  ...
  update(p, world, dt) {
    ...
    this.x = Math.max(0, Math.min(Math.max(0, world.width - 400), this.x));
    this.y = Math.max(0, Math.min(Math.max(0, 15 * 16 - 240), this.y));
...
export class Game {
  constructor(canvas) {
    ...
    this.input = new Input(); this.audio = new AudioEngine(); this.renderer = new Renderer(canvas);
    this.camera = new Camera(); this.particles = new Particles(); this.enemyAI = new EnemyAI();
    this.bullets = []; this.world = null; this.worldDef = null; this.player = null; this.levelIndex = 0;
    this.last = 0; this.acc = 0; this.step = 1 / 60; this.running = false;
    this.bindUI(); addEventListener('resize', () => this.renderer.resize()); this.renderer.resize(); this.recoverFocus();
  }
  ...
  bindUI() {
    ...
    if (restart) restart.addEventListener('click', () => this.start());
    if (resume) resume.addEventListener('click', () => { state.set({ screen: 'playing' }); this.running = true; this.last = performance.now(); requestAnimationFrame(t => this.loop(t)); });
    if (next) next.addEventListener('click', () => { this.levelIndex = Math.min(WORLDS.length - 1, this.levelIndex + 1); this.start(); });
  }

  start() {
    try {
      const def = WORLDS[this.levelIndex];
      if (!def) throw new Error(`Fase ${this.levelIndex + 1} não encontrada.`);
      this.worldDef = def;
      ...
      state.set({ screen: 'playing', world: this.levelIndex, coins: 0, time: def.time, lives: Math.max(1, state.data.lives) });
      this.audio.init(state.data.muted);
      this.running = true;
      this.acc = 0;
  ...
  lose() {
    ...
    this.player.state = PLAYER_STATES.DIE; this.player.body.vy = -250; state.data.lives--; state.save();
    this.audio.sfx('die'); this.camera.hit(7);
    setTimeout(() => { if (state.data.lives > 0) this.start(); else { this.running = false; state.set({ screen: 'gameover' }); } }, 850);
  }

  win() { state.data.score += 1000; state.save(); state.set({ screen: 'victory' }); this.audio.sfx('power'); this.running = false; }
  ...
  items(dt) {
    ...
    for (const it of this.world.items) { if (it.hidden) continue; if (aabb(this.player.body, it)) { it.hidden = true; this.player.power = it.type === 'flower' ? 'fire' : 'big'; this.player.state = PLAYER_STATES.POWERUP; this.audio.sfx('power'); this.particles.burst(this.player.body.x, this.player.body.y, 15, '#ffcc00', 120); } }
  }
  ...
  enemies(dt) {
    for (const e of this.world.enemies) {
      ...
      if (aabb(this.player.body, e) && this.player.inv <= 0) {
        ...
        else if (this.player.power !== 'small') { this.player.power = 'small'; this.player.inv = 1.3; this.audio.sfx('stomp'); }
        else this.lose();
      }
    }
  }
  ...
  update(dt) {
    ...
    this.input.update(); state.data.time = Math.max(0, state.data.time - dt);
    ...
    if (this.player.body.y > 260) this.lose(); if (this.player.body.x >= this.worldDef.goal) this.win();
  }

  render() {
    if (!this.world || !this.worldDef || !this.player) {
      // Não tenta renderizar a fase antes de ela existir.
      return;
    }
    this.renderer.draw(this.world, this.player, this.camera, this.particles.a, this.bullets, state.data.time);
    const setText = (id, value) => { const el = document.getElementById(id); if (el) el.textContent = value; };
    setText('scoreDisplay', String(state.data.score).padStart(6, '0'));
    setText('coinDisplay', 'x' + String(state.data.coins).padStart(2, '0'));
    setText('timeDisplay', Math.ceil(state.data.time));
    setText('livesDisplay', 'x' + String(state.data.lives).padStart(2, '0'));
    setText('worldDisplay', this.worldDef.id);
    const screens = { menu: 'menu', pause: 'pause', gameover: 'gameOver', victory: 'victory' };
    for (const [screen, id] of Object.entries(screens)) {
      const el = document.getElementById(id); if (el) el.classList.toggle('hidden', state.data.screen !== screen);
    }
  }
  ...
  loop(now) {
    try {
      ...
      const frame = Math.min(.25, Math.max(0, (now - this.last) / 1000)); this.last = now; this.acc += frame;
